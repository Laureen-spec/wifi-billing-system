...
const getImagePath = (path: string, pageProps?: any): string => {
  ...
  // Handle package paths or storage/media paths - use baseUrl directly
  if (path.includes('packages/workdo') || path.includes('packages/studyroomtechlab') || path.includes('storage/media')) {
    let baseUrl;
