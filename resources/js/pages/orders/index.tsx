                ...
                {chips.map((chip) => {
                    ...
                    return (
                        <button
                            ...
                            className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-medium transition ${selected ? 'border-emerald-500 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-border bg-background text-muted-foreground hover:border-emerald-200 hover:bg-emerald-50/60 hover:text-foreground'}`}
                        >
                            {chip.label}
                            <span className={`rounded-full px-2 py-0.5 text-xs ${selected ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'}`}>{count}</span>
                        </button>
                    );
                })}
            </div>
