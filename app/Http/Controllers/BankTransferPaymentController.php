...
class BankTransferPaymentController extends Controller
{
    public function store(StoreBankTransferPaymentRequest $request)
    {
        ...
        $baseSubscriptionPrice = $plan_price + $user_module_price + $user_price + $storage_price;
        $discountAmount = 0.0;
        $subscriptionPrice = $baseSubscriptionPrice;
        
        if ($request->coupon_code) {
            $validation = applyCouponDiscount($request->coupon_code, $baseSubscriptionPrice, auth()->id());
            if ($validation['valid']) {
                $discountAmount = (float) ($validation['discount_amount'] ?? 0);
                $subscriptionPrice = (float) ($validation['final_amount'] ?? max(0, $baseSubscriptionPrice - $discountAmount));
            }
        }

        $hotspotInvoice = $this->hotspotRevenueInvoiceForUser(Auth::user(), $plan);
        $price = max(0, $subscriptionPrice) + (float) $hotspotInvoice['hotspot_fee_amount'];
        ...
        $post['invoice'] = [
            'base_subscription' => $baseSubscriptionPrice,
            'discount_amount' => $discountAmount,
            'subscription_amount' => $subscriptionPrice,
            'hotspot_revenue' => $hotspotInvoice['hotspot_revenue'],
            'hotspot_fee_percent' => $hotspotInvoice['hotspot_fee_percent'],
            'hotspot_fee_amount' => $hotspotInvoice['hotspot_fee_amount'],
            'total_due' => $price,
            'invoice_lines' => [
                ['label' => 'Base subscription', 'amount' => $baseSubscriptionPrice],
                ['label' => 'Coupon discount', 'amount' => -1 * $discountAmount],
                ['label' => 'Hotspot revenue generated', 'amount' => $hotspotInvoice['hotspot_revenue'], 'display_only' => true],
                ['label' => 'Hotspot revenue fee (' . $hotspotInvoice['hotspot_fee_percent'] . '%)', 'amount' => $hotspotInvoice['hotspot_fee_amount']],
            ],
        ];
        ...
        $bank_transfer_payment->price_currency = getAdminAllSetting()['defaultCurrency'] ?? 'KES';
                ...
                if ($request->status == 'approved') {
                    ...
                    if ($assignPlan['is_success']) {
                        $order = Order::create([
                            ...
                            'discount_amount' => (float) data_get($requests, 'invoice.discount_amount', 0),
    ...
    private function hotspotRevenueInvoiceForUser(?User $user, ?Plan $plan): array
    {
        if (! $user) {
            return [
                'hotspot_revenue' => 0.0,
                'hotspot_fee_percent' => $this->planHotspotFee($plan),
                'hotspot_fee_amount' => 0.0,
            ];
        }
