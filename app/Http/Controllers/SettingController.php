...
class SettingController extends Controller
{
    ...
    public function updateCompanySettings(Request $request)
    {
        if(Auth::user()->can('edit-company-settings'))
        {
            ...
            if (!empty($settings['defaultCurrency'])) {
                $currencies = collect(config('default_currency.currencies', []));
                $selectedCurrency = $currencies->firstWhere('code', $settings['defaultCurrency']);
                if ($selectedCurrency) {
                    $settings['currencySymbol'] = $settings['currencySymbol'] ?? ($selectedCurrency['symbol'] ?? $settings['defaultCurrency']);
                }
            }
    ...
    {
        if(Auth::user()->can('edit-system-settings') || Auth::user()->can('edit-currency-settings'))
        {
            $request->validate([
    ...
    public function updateCurrencySettings(Request $request)
    {
        if(Auth::user()->can('edit-system-settings'))
        {
            $request->validate([
                ...
                'settings.currencySymbol' => 'required|string|max:20',
            ], [
                ...
                'settings.currencySymbol.required' => __('Currency symbol is required.'),
                'settings.currencySymbol.max' => __('Currency symbol must not exceed 20 characters.'),
            ]);
