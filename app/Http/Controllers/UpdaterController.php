...
class UpdaterController extends Controller
{
    ...
    private function getPendingMigrations()
    {
        try {
            ...
            // Package migrations: include both original Workdo packages and StudyRoomTechLab add-on packages.
            $packageDirs = array_merge(
                glob(base_path('packages/workdo/*/src/Database/Migrations'), GLOB_ONLYDIR) ?: [],
                glob(base_path('packages/studyroomtechlab/*/src/Database/Migrations'), GLOB_ONLYDIR) ?: []
            );

            foreach (array_unique($packageDirs) as $dir) {
                $files = glob($dir . '/*.php') ?: [];
                foreach ($files as $file) {
                    $allMigrations[] = basename($file, '.php');
                }
