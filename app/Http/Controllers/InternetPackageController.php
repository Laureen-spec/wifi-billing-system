...
class InternetPackageController extends Controller
{
    public function index(Request $request)
    {
        ...
        return Inertia::render('wifi-billing/plans/index', [
            'packages' => $packagesQuery
                ->latest()
                ->paginate(15)
                ->withQueryString()
                ->through(fn (InternetPackage $package): array => [
                    'id' => $package->id,
                    'name' => $package->name,
                    'package_type' => $package->package_type ?? $package->access_type,
                    ...
                    'enable_burst' => $package->enable_burst,
                    'enable_schedule' => $package->enable_schedule,
                    'available_on_all_mikrotik' => $package->available_on_all_mikrotik,
                    'hidden_from_client' => $package->hidden_from_client,
                    'routers_count' => $package->mikrotikRouters->count(),
                    'isp' => $package->isp ? [
                        'id' => $package->isp->id,
                        'name' => $package->isp->name,
                    ] : null,
                    'edit_url' => route('isp.packages.edit', $package),
                ]),
