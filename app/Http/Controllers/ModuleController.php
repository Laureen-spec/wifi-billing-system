...
class ModuleController extends Controller
{
    ...
    public function enable(Request $request)
    {
        ...
        if (!empty($module)) {
            $modulePath = (new Module())->packagePath($request->name);
    ...
    {
        $path = (new Module())->packagePath($module->name, 'module.json');
        ...
        if (isset($json['parent_module']) && !empty($json['parent_module'])) {
            foreach ($json['parent_module'] as $key => $value) {
                ...
                if ($this->isParentModuleEnabledGlobally($value)) {
                    $module = (new Module())->find($value);
                    if ($module) {
                        $parentCheck = $this->Check_Parent_Module($module);
                        if (($parentCheck['status'] ?? true) === false) {
                            return $parentCheck;
                        }
                    }
                } else {
    ...
    private function isParentModuleEnabledGlobally(string $moduleName): bool
    {
        ...
        return AddOn::where(function ($query) use ($moduleName, $module) {
                $query->where('module', $moduleName)
                    ->orWhere('package_name', $moduleName);

                if (! empty($module->name)) {
                    $query->orWhere('module', $module->name);
                }

                if (! empty($module->package_name)) {
                    $query->orWhere('package_name', $module->package_name)
                        ->orWhere('module', $module->package_name);
                }
            })
            ->where('is_enable', 1)
            ->exists();
    ...
    private function getAllModules()
    {
        ...
        $directories = (new Module())->getDirectories();
            ...
            if (File::exists($moduleJsonPath)) {
                ...
                if ($moduleData) {
                    ...
                    $modules[] = [
                        ...
                        'image' => (new Module())->packageUrl($moduleName, 'favicon.png'),
    ...
    private function installSinglePackage($file)
    {
        ...
        $tempPath = base_path('packages/tmp_' . uniqid());
        ...
        $extractPath = $this->getInstallPackagePath($moduleData, $tempPath . '/' . $rootFolderName);
            ...
            foreach ($userActiveModules as $userModule) {
                ...
                if ($addon) {
                    $modules[] = [
                        ...
                        'alias' => $addon->name,
                        'image' => (new Module())->packageUrl($userModule->module, 'favicon.png'),
    ...
    private function relativePackagePath(string $packagePath, string $append): string
    {
        return '/' . str_replace('\\', '/', trim(str_replace(base_path(), '', $packagePath), DIRECTORY_SEPARATOR)) . '/' . $append;
    }

    private function getInstallPackagePath(array $moduleData, string $sourcePath): string
    {
        $composerPath = $sourcePath . '/composer.json';
        $targetRoot = 'packages/workdo';
        ...
        if (File::exists($composerPath)) {
            ...
            foreach ($namespaces as $namespace) {
                if (str_starts_with($namespace, 'StudyRoomTechLab\\')) {
                    $targetRoot = 'packages/studyroomtechlab';
                    break;
                }
            }
        }

        return base_path($targetRoot . '/' . $moduleData['name']);
    }
}
