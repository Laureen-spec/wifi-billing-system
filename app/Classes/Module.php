...
class Module
{
    ...
    {
        $path = $this->packagePath($name, 'module.json');
        if (!File::exists($path)) {
            return false;
        }
    ...
    public function packagePath($name, $append = null)
    {
        foreach ($this->packageRoots as $root) {
            $path = base_path($root . '/' . $name);
            if (File::exists($path)) {
        ...
        foreach ($this->getDirectories() as $directory) {
            ...
            if (($moduleData['name'] ?? basename($directory)) === $name || ($moduleData['package_name'] ?? null) === $name) {
                return $append ? $directory . '/' . $append : $directory;
            }
        }

        return base_path('packages/workdo/' . $name . ($append ? '/' . $append : ''));
    }

    public function packageUrl($name, $append = null)
    {
        $path = $this->packagePath($name);
        $relativePath = str_replace('\\', '/', str_replace(base_path() . DIRECTORY_SEPARATOR, '', $path));

        return url('/' . $relativePath . ($append ? '/' . ltrim($append, '/') : ''));
    }
            ...
            function () use ($name) {
                if ($name === 'general') {
                    ...
                    if ($addonJson) {
                        ...
                        $this->image = $this->addon->image ?? $this->packageUrl($name, 'favicon.png');
                    ...
                    if ($this->addon) {
                        ...
                        $this->image = $this->addon->image ? getImageUrlPrefix().'/'.$this->addon->image : $this->packageUrl($name, 'favicon.png');
    ...
    public function getDirectories()
    {
        ...
        foreach ($this->packageRoots as $root) {
            ...
            foreach (File::directories($path) as $directory) {
                ...
                if (File::exists($moduleJsonPath)) {
                    $moduleData = json_decode(File::get($moduleJsonPath), true);
                    $moduleKey = $moduleData['name'] ?? $moduleKey;
                }

                $moduleKey = strtolower((string) $moduleKey);

                if (isset($seenModules[$moduleKey])) {
                    continue;
                }
    ...
    {
         $directories = array_unique(array_map(function ($dir) {
            return basename($dir);
        }, $this->getDirectories()));
