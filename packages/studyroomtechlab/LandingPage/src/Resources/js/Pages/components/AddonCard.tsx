...
export default function AddonCard({ addon, colors, priceType = 'monthly', variant = 'card1', onViewDetails }: AddonCardProps) {
    ...
    const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
        ...
        target.src = getImagePath(`/packages/StudyRoomTechLab/${addon.module}/favicon.png`);
        target.className = config.fallbackClass;
    };

    const renderImage = () => (
        <img
            src={getImagePath(addon.image ?? `/packages/StudyRoomTechLab/${addon.module}/favicon.png`)}
            alt={addon.name}
