...
const getPackageMenuItems = (userRoles: string[], activatedPackages: string[], t: (key: string) => string): NavItem[] => {
    ...
    activatedPackages.forEach(packageName => {
        const module = packageRoots
            .map((packageRoot) => allModules[`../../../packages/${packageRoot}/${packageName}/src/Resources/js/menus/${menuType}.ts`] as any)
            .find(Boolean);
...
const visibilityCandidates = (item: NavItem): string[] => {
    const menuItem = item as NavItem & { menuKey?: string };
    const candidates = [
        menuItem.menuKey,
        item.name,
        item.title,
    ...
    return Array.from(new Set(candidates.map(normalizeVisibilityKey).filter(Boolean)));
};

const filterByVisibility = (items: NavItem[], hiddenKeys: string[]): NavItem[] => {
    if (!hiddenKeys.length) {
        return items;
    }

    const hidden = new Set(hiddenKeys.map(normalizeVisibilityKey).filter(Boolean));
    ...
    return items.reduce<NavItem[]>((visibleItems, item) => {
        ...
        const isHidden = candidates.some(candidate => hidden.has(candidate));
        ...
        if (item.children) {
            nextItem.children = filterByVisibility(item.children, hiddenKeys);
...
export const allMenuItems = (): NavItem[] => {
    ...
    const visibilityFilteredItems = filterByVisibility(sortedMenuItems, hiddenMenuKeys);
